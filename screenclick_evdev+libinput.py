...
def monitor_input_device(device, is_touchscreen, calibration):
    ...
    # 当前坐标
    current_x = screen_width // 2  # 初始化为屏幕中心
    ...
    try:
        device.grab()  # 独占设备，可能需要root权限
        print("成功独占设备")
    except Exception as e:
        print(f"无法独占设备: {e}")
        print("继续监控但可能与系统操作冲突")
            ...
            try:
                for event in device.read():
                    ...
                    # 处理按钮事件
                        ...
                        # 鼠标左键或触摸屏点击
                        if event.code in [ecodes.BTN_LEFT, ecodes.BTN_TOUCH]:
                            # 映射到屏幕坐标
                            screen_x, screen_y = map_coordinates(current_x, current_y, 
...
def main():
    ...
    # 可选：创建状态文件，以便其他程序读取
    status_file = "/tmp/mouse_click_status.json"
